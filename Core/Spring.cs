...
namespace SpringAnimation.Core
{
    ...
    public class Spring
    {
        public static List<Tuple<SpringFunction, string>> SpringFunctionsAndNames
        {
            get
            {
                ...
                var staticPublicMethods = typeof(Spring).GetMethods(BindingFlags.Static | BindingFlags.Public);
        ...
        {
            // TODO: Fuzzy equals
            return (instant == end) ? (start + end) : end * (-Math.Pow(2, -10 * instant / duration) + 1) + start;
        }

        public static double ExponentialEaseIn(double start, double end, double instant, double duration)
        {
            // TODO: Fuzzy equals
            return (instant == 0) ? start : end * Math.Pow(2, 10 * (instant / duration - 1)) + start;
        }
